...
exports.reportdownload = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    console.log("i am downladed called");
    try {
        const data = yield expenses.findAll({ where: { UserId: req.user.dataValues.id } });
        const stringifieddata = JSON.stringify(data);
        const filename = `Expense${req.user.dataValues.id}/${new Date()}.txt`;
        const fileurl = yield Uploadfiles(stringifieddata, filename);
        console.log(fileurl);
